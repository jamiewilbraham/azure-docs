...
Use these container recipes to create Cognitive Research Technologies Containers that can be reused. Containers can be built with some or all configuration settings so that they are _not_ needed when the container is started.
...
* If someone gets access to your container registry or your local host, they can run the container and use the Cognitive Research Technologies endpoints.
* If your Cognitive Service doesn't require input mounts, don't add the `COPY` lines to your Dockerfile.

Create Dockerfile, pulling from the existing Cognitive Research Technologies container you want to use, then use docker commands in the Dockerfile to set or pull in information the container needs.
